...
if save:
    ...
    SaveFiles(folder_name, yamlfile, p, sv_save, user_inputs)
...
iO2_n = ca.naf_b.species_index('O2(Naf)')
...
if debug:
    ...
    plt.ylabel(r'Cathode DL Potential [V]')
    plt.xlabel('Time, t [s]')
    plt.tight_layout()
    ...
    for i in range(Ny_cl):
        ...
        plt.ylabel(r'Nafion $\rho_k$ [kg/m$^3$]')
        plt.xlabel('Time, t [s]')
        plt.tight_layout()
...
if radial:
    ...
    plt.ylabel(ca.naf_b.species_names[iO2_n]+' [kg/m$^3$]')
...
if grads:
    ...
    plt.ylabel(ca.gas.species_names[iO2_g]+'(gas) [kg/m$^3$]')
    plt.tight_layout()
    ...
    plt.xlabel(r'Cathode CL Depth [$\mu$m]')
    plt.ylabel(ca.naf_b.species_names[iO2_n]+' at r=R [kg/m$^3$]')
    plt.tight_layout()
...
if i_ver:
    ...
    O2_BC_flux = fickian_adf(TDY1, TDY2, ca.gas, gdl_cl, gdl_cl, gdl_cl) \
