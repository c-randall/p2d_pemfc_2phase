...
# Species indexes:
iO2_g = ca.gas.species_index('H2O')
iO2_n = ca.naf_b.species_index('H2O(Naf)')
iH_n = ca.naf_b.species_index('H(Naf)')

# Toggle for shell indecies if using flooded_agg:
start_tog, shl_tog = 'rho_naf_k', 0
...
if debug:
    ...
    for i in range(Ny_cl):
        ...
        ind1 = ca.ptr[start_tog][iO2_n] +i*cl['nxt_y']
        ind2 = ca.ptr[start_tog][iO2_n] +i*cl['nxt_y'] +(Nr_cl +shl_tog)*cl['nxt_r']
        species_k_Naf = sol.y[ind1:ind2:cl['nxt_r'], :]

        for j in range(Nr_cl +shl_tog):
            plt.figure(fig_num)
            plt.plot(sol.t, species_k_Naf[j])

            legend_str.append(ca.naf_b.species_names[iO2_n] + 'r' + str(legend_count))
            legend_count = legend_count +1

        plt.title('y-node = ' + str(i))
        plt.legend(legend_str)
        plt.ylabel(r'Nafion Phase Densities [kg/m$^3$]')
        plt.xlabel('Time, t [s]')
        plt.tight_layout()
    ...
    for i in range(ca.gas.n_species):
        species_gas_cl = sol.y[ca.ptr['rho_gas_k'][i]:L_sv:cl['nxt_y']][0]
        species_gas_gdl = sol.y[ca.ptr['rho_gdl_k'][i]:L_gdl:gdl['nxt_y']][0]
    ...
    plt.figure(fig_num)
    plt.legend(ca.gas.species_names)
    plt.ylabel(r'CL Gas Phase Densities [kg/m$^3$]')
    plt.xlabel('Time, t [s]')
    plt.tight_layout()
    
    plt.figure(fig_num+1)
    plt.legend(ca.gas.species_names)
    plt.ylabel(r'GDL Gas Phase Densities [kg/m$^3$]')
    plt.xlabel('Time, t [s]')
    plt.tight_layout()
    ...
    for i in range(Ny_cl):
        ...
        for j in range(ca.pt_s.n_species):
            theta_pt_k = sol.y[ca.ptr['theta_pt_k'][j] +i*cl['nxt_y'], :]
        ...
        fig_num = fig_num +1
        plt.title('y-node = ' + str(i))
        plt.legend(ca.pt_s.species_names)
...
if radial:
    ...
    for i in range(Ny_cl):
        ind_1 = int(ca.ptr[start_tog][iO2_n] +i*cl['nxt_y'])
        ind_2 = int(ca.ptr[start_tog][iO2_n] +i*cl['nxt_y'] +(Nr_cl +shl_tog)*cl['nxt_r'])
        naf_O2_r = sol.y[ind_1:ind_2:cl['nxt_r'], -1]
        
        ind_1 = int(ca.ptr[start_tog][iH_n] +i*cl['nxt_y'])
        ind_2 = int(ca.ptr[start_tog][iH_n] +i*cl['nxt_y'] +(Nr_cl +shl_tog)*cl['nxt_r'])
        naf_H_r = sol.y[ind_1:ind_2:cl['nxt_r'], -1]

        plt.figure(fig_num)
        plt.plot(1/cl['1/r_j']*1e9, naf_O2_r, '-o')
        #plt.plot(1/cl['1/r_j']*1e9, naf_H_r, '-s')
        legend_str.append('y node = ' + str(legend_count))
        legend_count = legend_count +1

    plt.xlabel('Nafion Shell Radius [nm]')
    plt.ylabel(ca.naf_b.species_names[iO2_n]+' Density [kg/m$^3$]')
    plt.legend(legend_str, loc='best')
    plt.tight_layout()

    if save:
        plt.savefig('Nafion_'+ca.naf_b.species_names[iO2_n]+'_v_Radius.png')
...
if grads:
    ...
    plt.figure(fig_num)
    Phi_elyte_y = -1*sol.y[ca.ptr['phi_dl']:L_sv:cl['nxt_y'], -1]
    ...
    rho_O2_gdl = sol.y[ca.ptr['rho_gdl_k'][iO2_g]:L_gdl:gdl['nxt_y'], -1]
    rho_O2_CL = sol.y[ca.ptr['rho_gas_k'][iO2_g]:L_sv:cl['nxt_y'], -1]
    ...
    plt.ylabel(ca.gas.species_names[iO2_g]+'(gas) Density [kg/m$^3$]')
    plt.tight_layout()

    if save:
        plt.savefig('Gas_Phase_'+ca.gas.species_names[iO2_g]+'_v_Depth.png')
        
    fig_num = fig_num +1

    # Check O2 gradient direction - naf phase (outermost shell):
    plt.figure(fig_num)
    rho_O2_y = sol.y[int(ca.ptr[start_tog][iO2_n]):L_sv:cl['nxt_y'], -1]
    plt.plot(np.linspace(0,t_cl*1e6,Ny_cl), rho_O2_y, '-o')

    plt.xlabel(r'Cathode CL Depth [$\mu$m]')
    plt.ylabel(ca.naf_b.species_names[iO2_n]+' Density (outer R) [kg/m$^3$]')
    plt.tight_layout()
    ...
    for i in range(cl['Ny']):
        ...
        i_far_cl[i] = ca.pt_s.get_net_production_rates(ca.carb)\
...
if polar:
    ...
    if all([data, w_Pt == 0.2]):
        ...
        y = np.array([0.952, 0.849, 0.803, 0.772, 0.731, 0.716, 0.700, 0.675, 
                      0.665, 0.647, 0.634])
        yerr = np.array([0.001, 0.012, 0.007, 0.007, 0.012, 0.001, 0.008, 0.007,
                         0.007, 0.009, 0.009])
        color, legend = 'C0', 1
        ...
        y = np.array([0.930, 0.834, 0.785, 0.754, 0.711, 0.691, 0.673, 0.649, 
                      0.635, 0.615, 0.598])
        yerr = np.array([0.001, 0.009, 0.007, 0.005, 0.007, 0.011, 0.011, 0.007, 
                         0.009, 0.011, 0.011])
        color, legend = 'C1', 0
        ...
        y = np.array([0.919, 0.810, 0.760, 0.724, 0.674, 0.653, 0.634, 0.603,
                      0.585, 0.558, 0.537])
        yerr = np.array([0.001, 0.008, 0.006, 0.006, 0.007, 0.007, 0.005, 0.005, 
                         0.006, 0.007, 0.007])
        color, legend = 'C2', 0
        ...
        y = np.array([0.910, 0.785, 0.724, 0.683, 0.626, 0.598, 0.572, 0.527, 
                      0.502, 0.463, 0.430])
        yerr = np.array([0.001, 0.004, 0.010, 0.014, 0.013, 0.013, 0.019, 0.024, 
                         0.025, 0.023, 0.024])
        color, legend = 'C3', 0
    elif not data:
        x = y = yerr = None
    ...
    try:
        ...
        if data: plt.ylim([0.35, 1.0]); plt.xlim([0, 2.1])
...
if i_ver:
    ...
    TDY2 = cl['T'], sum(rho_cl_k), rho_cl_k

    O2_BC_flux = fickian_adf(TDY1, TDY2, ca.gas, gdl_cl, gdl_cl, gdl_cl, 1) \
              / ca.gas.molecular_weights

    print('\ni_ext:', np.round(i_ext[i_ind],3))
